...
public class LocalFtpWrapper extends FTPClient {
	
	private static String Local_FTP_serverIP = CommonDA.getProperties().getProperty("Local_FTP_serverIP").trim();
	private static String Local_FTP_username = CommonDA.getProperties().getProperty("Local_FTP_username").trim();
	private static String Local_FTP_password = CommonDA.getProperties().getProperty("Local_FTP_password").trim();

	private String serverName;
	private String username;
	private String password;

	public LocalFtpWrapper() {
		super();
...
//			p1.load(this.getClass().getClassLoader().getResourceAsStream("/com/windworld/files/dbConfigurationFile/ftpServer.properties"));
			setServerName(Local_FTP_serverIP);
			setUsername(Local_FTP_username);
			setPassword(Local_FTP_password);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
